...
int counter = 0 ;
int task_d[4] = {0,0,0,0} ;
int is_busy[4] = {0,0,0,0};
int counts = 0 ;
#define worker_c  4 
#define BUSY  0  
#define REST  1 
 ...
 */
tpool_future_t tpool_apply(tpool_t pool, void *(*func)(void *), void *arg , int id);
    ...
    while (1) {
        ...
        if (task->func) {
            ...
            if (task->future->flag & __FUTURE_DESTROYED) {
                ...
                //printf("This is worker %u doing task id : %u\n" ,w_id ,task->id) ;
                task_d[w_id]++;
...
struct __threadpool *tpool_create(size_t count)
{
    ...
    jobqueue_t *head  = jobqueue_create() ;
    if ( !head || !pool) {
        free(pool);
        if(!head)
          free(head) ;
    ...
    pool->count = count, pool->jobqueue = head;
                                  ...
                                  )
{
    ...
    if (new_head && future) {
        new_head->func = func, new_head->arg = arg, new_head->future = future ,new_head->id = id;
        if(func){
            for(;;jobqueue = jobqueue->next){
                if(jobqueue->busy == REST)
                break ;
            }
        }
        else{
...
int main()
{
    ...
    double bpp_sum = 0;
    tpool_t pool = tpool_create(worker_c);
    tpool_future_t futures[PRECISION + 1];

    for (int i = 0; i <= PRECISION; i++) {
        bpp_args[i] = i;
        futures[i] = tpool_apply(pool, bpp, (void *) &bpp_args[i] , i);
    }
